import { EXPORT_FORMAT, EXPORT_VERSION, SCHEMA_VERSION, STORE_NAMES, createMetadataRecord, createRoadmapRecord, isConversationRecord, isEndOfDayRecord, isIssueRecord, isKpiEntryRecord, isKpiRecord, isMetadataRecord, isMorningHuddleRecord, isRoadmapRecord, isSupportedExport, isTaskRecord, nowIso } from './schema.js';
import { getAllRecords, replaceStoreRecords } from './storage.js';

const DATA_STORES = Object.values(STORE_NAMES);
const EMPTY = Object.freeze({ metadata: [], tasks: [], endOfDay: [], morningHuddles: [], roadmap: [], settings: [], kpis: [], kpiEntries: [], conversations: [], issues: [] });
...
export function validateImport(envelope) {
  ...
  const importedMetadata = Array.isArray(source.metadata) && source.metadata.length === 1 && source.metadata[0]?.schemaVersion === SCHEMA_VERSION ? source.metadata : [createMetadataRecord(envelope.exportedAt)];
  const data = { ...EMPTY, ...source, metadata: importedMetadata, roadmap: source.roadmap || [createRoadmapRecord({}, { now: envelope.exportedAt })], kpis: source.kpis || [], kpiEntries: source.kpiEntries || [], conversations: source.conversations || [], issues: source.issues || [] };
  ...
  data.issues.forEach(record => referencesExist(record.linkedTaskIds || [], 'issue'));
