...
export const STORE_NAMES = Object.freeze({ metadata: 'metadata', tasks: 'tasks', endOfDay: 'endOfDay', morningHuddles: 'morningHuddles', roadmap: 'roadmap', settings: 'settings', kpis: 'kpis', kpiEntries: 'kpiEntries', conversations: 'conversations', issues: 'issues' });
...
export const ISSUE_STATUSES = Object.freeze(['open', 'prioritised', 'in-progress', 'waiting', 'resolved', 'verified']);
export const ISSUE_PRIORITIES = Object.freeze(['low', 'normal', 'high', 'urgent']);
export function createIssueRecord(input, { id = createId(), now = nowIso() } = {}) {
  const title = cleanText(input?.title);
  if (!title) throw new TypeError('An issue title is required.');
  const status = ISSUE_STATUSES.includes(input?.status) ? input.status : 'open';
  const ids = cleanIds(input?.linkedTaskIds);
  const actions = (Array.isArray(input?.correctiveActions) ? input.correctiveActions : []).filter(action => action && typeof action === 'object').map(action => ({ id: cleanText(action.id) || createId(), title: cleanText(action.title), taskId: cleanText(action.taskId) || null })).filter(action => action.title || action.taskId);
  const why = (Array.isArray(input?.fiveWhys) ? input.fiveWhys : []).slice(0, 5).map(value => cleanText(value)).filter(Boolean);
  const refs = input?.relatedRefs || {};
  return { id, title, context: cleanText(input?.context ?? input?.description), category: cleanText(input?.category), priority: ISSUE_PRIORITIES.includes(input?.priority) ? input.priority : 'normal', status, impact: cleanText(input?.impact), ownerRole: cleanText(input?.ownerRole), createdAt: input?.createdAt ?? now, updatedAt: input?.updatedAt ?? now, prioritisedAt: input?.prioritisedAt ?? (status !== 'open' ? now : null), resolvedAt: input?.resolvedAt ?? (['resolved', 'verified'].includes(status) ? now : null), verifiedAt: input?.verifiedAt ?? (status === 'verified' ? now : null), decisionStatement: cleanText(input?.decisionStatement), decisionRationale: cleanText(input?.decisionRationale), decisionDate: cleanText(input?.decisionDate) || null, fiveWhys: why, resolutionSummary: cleanText(input?.resolutionSummary), verificationOutcome: cleanText(input?.verificationOutcome), reopenReason: cleanText(input?.reopenReason), linkedTaskIds: ids, correctiveActions: actions, relatedRefs: { kpiId: cleanText(refs.kpiId) || null, huddleId: cleanText(refs.huddleId) || null, eodId: cleanText(refs.eodId) || null, meetingId: cleanText(refs.meetingId) || null, roadmapMilestoneId: cleanText(refs.roadmapMilestoneId) || null } };
}
...
export function isIssueRecord(value) { try { const record = createIssueRecord(value, { id: value.id, now: value.createdAt }); return Boolean(value) && typeof value.id === 'string' && record.createdAt === value.createdAt && typeof value.updatedAt === 'string' && ISSUE_STATUSES.includes(record.status); } catch { return false; } }
