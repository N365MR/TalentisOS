...
export async function replaceStoreRecords(recordsByStore) { const stores = Object.keys(recordsByStore); stores.forEach(assertStore); const database = await openWorkspaceDatabase(); try { const transaction = database.transaction(stores, 'readwrite'); for (const storeName of stores) { const store = transaction.objectStore(storeName); store.clear(); recordsByStore[storeName].forEach(record => storeName === METADATA_STORE ? store.put(record, METADATA_KEY) : store.put(record)); } await transactionComplete(transaction); } finally { database.close(); } }
export async function clearWorkspaceData() { const stores = Object.values(STORE_NAMES); const database = await openWorkspaceDatabase(); try { const transaction = database.transaction(stores, 'readwrite'); stores.forEach(store => transaction.objectStore(store).clear()); await transactionComplete(transaction); } finally { database.close(); } await initializeWorkspaceStorage(); }
