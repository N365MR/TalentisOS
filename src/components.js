...
export function createEodView({ eod, date, workItems = [], history = [], filter = 'all' }) {
  ...
  const historyMarkup = history.length ? `<section class="eod-history"><div class="section-heading"><div><p class="eyebrow">Past close-outs</p><h2>EOD History</h2></div></div><div class="eod-history-list">${history.map((item) => `<article><div><strong>${escapeHtml(item.date)}</strong><span>${item.status === 'closed' ? 'Closed' : 'In progress'} · ${item.completedTaskIds?.length || 0} completed · ${item.riskIds?.length || 0} risks</span></div><button type="button" class="text-button" data-eod-history-id="${escapeHtml(item.id)}">View summary</button></article>`).join('')}</div></section>` : '';
