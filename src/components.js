...
export function createHuddleMeetingDialog(date, workItems, huddleItems) {
  const items = huddleItems.map((ref) => workItems.find((item) => item.id === ref.itemId)).filter((item) => item && item.status !== 'complete');
  return `<dialog id="morning-huddle-dialog" class="modal morning-huddle-dialog" aria-labelledby="morning-huddle-title"><div class="modal__header"><div><p class="eyebrow">Today · Prepare · Align</p><h2 id="morning-huddle-title">Morning Huddle</h2><p class="secondary-text">${escapeHtml(workDateLabel(date))} · ${items.length} carried-over item${items.length === 1 ? '' : 's'}</p></div><button class="icon-button" type="button" data-close-dialog aria-label="Close Morning Huddle">×</button></div><div class="modal__body"><section class="huddle-dialog-intro"><p>Decide what needs attention today. Complete it, add it to Today, block it, or move it forward.</p></section><section class="huddle-dialog-section" aria-labelledby="huddle-dialog-carried-title"><div class="section-heading"><div><p class="eyebrow">Continuity</p><h3 id="huddle-dialog-carried-title">Carried forward</h3></div><span class="section-count">${items.length}</span></div>${items.length ? `<div class="huddle-dialog-list">${items.map((item) => { const incomplete = (item.subtasks || []).filter((subtask) => !subtask.completed); return `<article class="huddle-dialog-item"><div><strong>${escapeHtml(item.title || item.outcome)}</strong><small>Raised ${escapeHtml(workDateLabel(item.raisedDate || item.createdAt?.slice(0, 10)))}${item.dueDate ? ` · Due ${escapeHtml(workDateLabel(item.dueDate))}` : ''}${item.blocked ? ' · Blocked' : ''}</small>${incomplete.length ? `<small>${incomplete.length} incomplete subtask${incomplete.length === 1 ? '' : 's'}</small>` : ''}</div><div class="huddle-dialog-item__actions"><button type="button" class="text-button" data-complete-work="${escapeHtml(item.id)}">Complete</button><button type="button" class="text-button" data-huddle-add-today="${escapeHtml(item.id)}">Add to Today</button></div></article>`; }).join('')}</div>` : '<div class="section-empty"><p>No carried-over items.</p><small>Add a new work item to start the day.</small></div>'}</section><div class="modal__actions"><button type="button" class="primary-action" data-huddle-dialog-add>＋ Add work item</button><button type="button" class="secondary-action" data-close-dialog>Continue to Huddle</button></div></div></dialog>`;
}
