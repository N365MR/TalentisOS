...
Phase 04 adds `src/state/morning-huddle.js` as the Huddle boundary. The view receives an assembled snapshot and does not query stores or calculate workdays. It consumes Phase 03 `getMorningHuddleInputs()` and canonical task operations. Phase 05's supported handoff is `getAlignedCommitments(workDate)`, returning canonical task IDs, Huddle/workday identity, commitment state, and live canonical tasks.
