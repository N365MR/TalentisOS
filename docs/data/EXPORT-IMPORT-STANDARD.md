...
Current exports use `{ format: "TalentisOS", exportVersion: 7, exportedAt, data: { metadata, tasks, endOfDay, morningHuddles, roadmap, settings, kpis, kpiEntries, conversations } }`. KPI and conversation records carry canonical task IDs only. Import validates identity, version, structures, record shapes, unique IDs and all practical canonical references before a confirmed full replacement transaction. Broken references are rejected rather than removed. Versions 1–6 may be normalized only by adding later empty stores/default singleton records; newer formats are rejected. Users should export before upgrades and retain offline backups. Local files are treated as untrusted input.
